...
@Component({
  ...
  styleUrls: ['./list-project-user.component.scss']
})
export class ListProjectUserComponent {
  ...
  ngOnInit() {
     const savedMode = this.cookieService.get('displayModeCl');
  ...
  isDateOverdue1(df: Date, drf: Date): boolean {
   ...
   setDisplayMode(mode: 'table' | 'grid') {
    ...
    this.cookieService.set('displayModeCl', mode, 365); // save for 1 year
  }
