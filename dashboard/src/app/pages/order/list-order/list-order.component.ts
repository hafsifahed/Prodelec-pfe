...
export class ListOrderComponent implements OnInit {
  ...
  constructor(
    private spartner: PartnersService,
  ...
  ngOnInit() {
    ...
    this.ordersForm = this.formBuilder.group({
      ...
      // numdev: supprimé car non utilisé
  ...
  private loadOrdersAfterChange() {
    // Recharge la liste des commandes sans reloader toute la page
    if (this.user) {
      this.orderservice.getAllOrders().subscribe({
        next: (data) => {
          this.list = data.filter((order) => !order.archivera);
    ...
    switch (httpEvent.type) {
      case HttpEventType.Response:
        if (httpEvent.body instanceof Blob) {
          ...
          console.error('Corps de réponse invalide. Blob attendu mais reçu:', typeof httpEvent.body);
        }
        break;
      default:
  ...
  toggleSection(modal: 'add' | 'edit', section: string) {
    if (modal === 'add') {
      switch (section) {
        case 'conception':
          this.showConceptionAdd = !this.showConceptionAdd;
          break;
        case 'methode':
          this.showMethodeAdd = !this.showMethodeAdd;
          break;
        case 'production':
          this.showProductionAdd = !this.showProductionAdd;
          break;
        case 'controle':
          this.showControleAdd = !this.showControleAdd;
          break;
        case 'livraison':
          this.showLivraisonAdd = !this.showLivraisonAdd;
          break;
      }
    }
  }
